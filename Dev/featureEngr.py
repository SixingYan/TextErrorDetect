...
def feature_engineering(X):
    ...
    X['sent'] = X['sent'].apply(lambda x: ''.join(w.strip() for w in re.findall(r'[\u4e00-\u9fa5]', x) if len(w.strip()) > 0))
    ...
    X['1n_ppl_p_jieba'] = X['sent'].progress_apply(lambda x: l1p.perplexity(x) if l1p.perplexity(x) != float('inf') else -1)
    X['1n_etp_p_jieba'] = X['sent'].progress_apply(lambda x: l1p.entropy(x) if l1p.entropy(x) != float('inf') else -1)

    X['2n_etp_p_jieba'] = X['sent'].progress_apply(lambda x: l2p.entropy(x) if l2p.entropy(x) != float('inf') else -1)
    X['2n_ppl_p_jieba'] = X['sent'].progress_apply(lambda x: l2p.perplexity(x) if l2p.perplexity(x) != float('inf') else -1)

    X['3n_etp_p_jieba'] = X['sent'].progress_apply(lambda x: l3p.entropy(x) if l3p.entropy(x) != float('inf') else -1)
    X['3n_ppl_p_jieba'] = X['sent'].progress_apply(lambda x: l3p.perplexity(x) if l3p.perplexity(x) != float('inf') else -1)
    ...
    X['2n_etp_n_jieba'] = X['sent'].progress_apply(lambda x: l2n.entropy(x) if l2n.entropy(x) != float('inf') else -1)
    X['2n_ppl_n_jieba'] = X['sent'].progress_apply(lambda x: l2n.perplexity(x) if l2n.perplexity(x) != float('inf') else -1)

    X['3n_etp_n_jieba'] = X['sent'].progress_apply(lambda x: l3n.entropy(x) if l3n.entropy(x) != float('inf') else -1)
    X['3n_ppl_n_jieba'] = X['sent'].progress_apply(lambda x: l3n.perplexity(x) if l3n.perplexity(x) != float('inf') else -1)
    ...
    #X['3n_etp_s1_jieba'] = X['sent'].progress_apply(lambda x: l3s1.entropy(x) if l3s1.entropy(x) != float('inf') else -1)
    #X['3n_ppl_s1_jieba'] = X['sent'].progress_apply(lambda x: l3s1.perplexity(x) if l3s1.perplexity(x) != float('inf') else -1)
...
def main():
    ...
    extract(const.DATAPATH, 'kenlm_paopao_chars.csv', 'data_kenlm_paopao_v2.csv')
